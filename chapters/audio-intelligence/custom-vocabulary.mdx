...
<Note>
If you already know exactly which *text* variants the model produces and you just want to normalize the spelling, use **[Custom spelling](/chapters/audio-intelligence/custom-spelling)** instead. Custom spelling is a deterministic find-and-replace on the transcript text, with no phoneme matching and no false positives.
</Note>
...
Once the transcription is generated, Gladia converts both the transcribed words and your vocabulary entries into phonemes, then compares them. If the similarity score crosses the intensity threshold you've set, the transcribed word is replaced with your entry.

The `pronunciations` field lets you provide **plain-text alternative spellings that reflect how the word actually sounds** in speech. These are *not* IPA or phonetic notation. Just write the word the way someone might naively spell it based on how it sounds. Gladia converts these strings to phonemes internally. For example, if your term is "Nietzsche", you might add `["Niche", "Neechee"]` as pronunciations. This widens the phoneme net without having to lower the intensity threshold (which would increase false positives across the board).
...
<ParamField body="default_intensity" type="number">
  The global intensity threshold applied to every vocabulary entry that doesn't have its own `intensity` override (minimum 0, maximum 1, default 0.5).

  A higher value means the transcribed word must sound *very* similar to your entry before it gets replaced: fewer replacements, fewer false positives. A lower value casts a wider net: more replacements, but more risk of unwanted swaps.
</ParamField>
<ParamField body="vocabulary" type="object">
  <Expandable title="properties">
    ...
    <ParamField body="intensity" type="number">
      The intensity threshold for this specific entry (minimum 0, maximum 1, default: inherits from `default_intensity`). Use this to make individual entries more or less aggressive than the global default. For example, set a higher intensity on a short word that keeps producing false positives.
    </ParamField>
...
**We recommend starting at 0.3–0.4** and increasing only if you notice that your terms are still not being picked up.
...
You can mix both. A common pattern: set `default_intensity` to 0.3, then bump individual short or common-sounding words (like "Arc" or "Via") up to 0.5–0.6 to avoid them matching too many unrelated words.
...
1. Raising the intensity for the entries that cause problems.
...
5. **Look for false positives**, words that were correct before but are now being wrongly replaced. If you spot any:
   - Raise the `intensity` on the entry causing the problem.
...
<Tip>
If custom vocabulary keeps producing false positives for a particular term no matter how you tune it, switch that term to **[custom spelling](/chapters/audio-intelligence/custom-spelling)**. Custom spelling is deterministic: if the model already outputs something close to the right text, custom spelling will normalize it without any risk of unintended replacements. This is a common migration path and the right call when phoneme matching is more trouble than it's worth for a given word.
</Tip>
