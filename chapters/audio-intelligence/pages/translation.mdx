...
The translation feature now supports enhanced word splitting functionality through the `split_words` parameter:

- This parameter can be activated in API calls to ensure Gladia doesn't group words but keep the word by word format. However the translation will not match the original prosody which can lead to desynchronization in lypsinc use case.
...
async function startTranscription() {
  ...
  const requestData = {
    ...
    translation_config: {
      ...
      split_words: true // Enables enhanced word splitting to remove extra spaces and adjust timing details
    }
  };
...
request_data = {
    ...
    "translation_config":
    {
      "target_languages": ['fr', 'es'],
      "model": "base", ## "enhanced" is slower but of better quality
      "split_words": True  ## Enables enhanced word level timing splits
    }
}
