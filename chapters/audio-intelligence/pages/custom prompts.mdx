...
The **Custom Prompts** feature apply your own prompts to the audio transcription.
...
async function pollForResult(resultUrl: string, headers: any) {
  while (true) {
    ...
    if (pollResponse.status === "done") {
      ...
      const customPromptsResults = pollResponse.result.custom_prompts;
      console.log(customPromptsResults);
      break;
    } else {
...
if result_url:
    while True:
        ...
        if poll_response.get("status") == "done":
            ...
            custom_prompts_results = poll_response.get("result", {}).get(
                "custom_prompts", {})
