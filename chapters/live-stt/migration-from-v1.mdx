...
Real-time V2 is the latest real-time speech-to-text API from Gladia. It offers more features and has significant improvements in latency compared to V1.
Here is a guide on to migrate to V2, so you can start enjoying all the benefits. Please make sure you migrate sooner rather than later as we're looking to remove support for V1 sometime in the future. Before we do so however, we'll of course reach out to those of you who are still on V1.&#x20;
...
In V2, you first generate a unique WebSocket URL with a call to our [POST /v2/live endpoint](/api-reference/v2/live/init), and then connect to it.
This URL contains a token that is unique to your live session. You'll be able to resume your session in case of lost connection, or give the URL to a web client without exposing your Gladia API key.
...
With V2 offering more features, the configuration comes with some changes. You can find the full configuration definition in the [POST /v2/live API reference page](/api-reference/v2/live/init).
...
<Note>Languages are now specified with a 2-letter code, as in the API for pre-recorded speech-to-text.<br />See [this page](/chapters/limits-and-specifications/languages) for a complete list of codes.</Note>
...
The parameter `frames_format` is hence no longer present.
...
They may return in another form in the feature.
...
In V1, we send only two kinds of messages through WebSocket:
...
In V2, you should confirm that the `type` field is `transcript` and `data.is_final` is `true`.
...
If you're not interested in new messages and simply want the ones from the V1 API, you can configure what kind of messages you want when calling the [POST /v2/live endpoint](/api-reference/v2/live/init) to initiate the session.
