...
```javascript Stop the recording
liveSession.stopRecording()
```

```python Stop the recording
...
```javascript Get the final results
...
```typescript Merge audio channels
...
```typescript Send multi-channel audio
...
```javascript Close the connection
socket.close(1000);
```
