...
In th first version of the Gladia API, to get your transcription through an HTTP call, you had to send everything
...
<Steps>
  ...
  <Step title="Transcribe">
    ...
    - **Old V1** : The HTTP connection is kept openned until you get your transcription result, and there's no third step.

    - **New V2** : You get an instant response from the request with and `id` and a `result_url`. The `id` is your transcription ID that you
    will use to get your transcription result once it's done. You don't have to keep any HTTP connection open on your side.
  ...
  <AccordionGroup>
    ...
    You can get more informations on the different transcriptions status by checking the dedicated [Speech Recognition part](/chapters/speech-to-text-api/pages/speech-recognition)
...
In addition the transcription flow & input changes, output also changed. 
...
To dive deeper into the V2 version of the API, please take a look at thoses next:
