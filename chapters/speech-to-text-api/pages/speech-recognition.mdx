...
## Custom spelling
You can ask the model to enforce specific spellings for words using the custom_spelling parameter. This ensures that certain words are spelled in the desired manner throughout the transcript, which can be particularly useful for names or terms with multiple possible spellings.

To use this feature, you need to provide a configuration dictionary through the custom_spelling_config parameter. This dictionary should contain the correct spelling as the key and a list of possible variations as the value.

This feature is useful in scenarios where consistent spelling of specific words is crucial (e.g., technical terms in industry-specific recordings).
...
In this example, the model will ensure that "Gorish" is spelled correctly throughout the transcript, even if it is pronounced in various ways such as "ghorish," "gaurish," or "gaureish." Similarly, "Data Science" will be consistently spelled, regardless of whether it is pronounced as "data-science" or "data science."

<Tip>The key is case-sensitive, but the value isn't. The value can contain multiple words.</Tip>
