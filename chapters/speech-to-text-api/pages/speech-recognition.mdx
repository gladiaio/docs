...
Discover our state-of-the-art ASR model [ Whisper Zero now.](https://www.gladia.io/whisper-zero)
 ...
 Additional capabilities, like Speaker Diarization, Summarization, Translation, and more can be integrated seamlessly
 into the transcription process by including extra parameters in the transcription request.


## Basic transcription

<Tip>Don't forget to update *YOUR_GLADIA_TOKEN* and *YOUR_AUDIO_URL* to match yours.</Tip>

// TODO : add example for users using audio files instead of urls with upload step
...
async function startTranscription() {
  const gladiaKey = "YOUR_GLADIA_TOKEN";
  const requestData = {
    audio_url:
      "YOUR_AUDIO_URL",
  };
...
## Get your transcription results
...
The first way is the one we used in the above examples.

Once you post your transcription request, you get a transcritpion `id` and a pre-built `result_url` for convenience.
To get this result with this method, you'll just have to GET continuously on the given `result_url` until the status
of your transcription is `done`.


// TODO: Link to the API reference & documented flow 
...
Instead of polling and keeping your server busy and mainting work, you can use the `callback_url` feature.
...
Once the transcription is document, we'll make a `POST` request to the `callback_url` you provided with the results.
...
If you get an error, instead of an `payload` property you'll get an `error` key of shape:
...
| `error`     | An error occured on your transcription   |
...
**Word-level timestamps** feature. It let you know the exact timestamp words are detected and give you a more precise transcription.
...
To enable it, set detect_language to true in the request body parameters.
...
You'll have your subtitles in the orginal language, and **also** in languages you targeted for the translation !
</Tip>
...
The `JSON` response will inculude a new property `subtitles` which is an array of every formats you requested.
