...
```javascript Transcribe an audio file
...
```javascript Upload an audio file
import { GladiaClient } from "@gladiaio/sdk";
...
```javascript Create a transcription job
import { GladiaClient } from "@gladiaio/sdk";
...
```python Create a transcription job
from gladiaio_sdk import GladiaClient
...
```javascript Get the transcription result
