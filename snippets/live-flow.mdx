...
It's important to correctly define the properties `encoding`, `sample_rate`, `bit_depth` and `channels` as we use them to parse your audio chunks.
...
You'll receive a response with a WebSocket URL to connect to. If you loose connection, you can reconnect to that same URL to resume where you left off. Here's an example of a response:
...
Here's an example of a [`transcript`](/api-reference/v2/live/message/transcript) message received through a WebSocket:
...
We will process remaining audio chunks and start the post-processing phase, in which we put together the final audio file and the add-ons you requested.
You'll receive a message at every step of the process in the WebSocket, or the callback, if configured.
